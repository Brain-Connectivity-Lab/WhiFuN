ants_bin = "/opt/ANTs/bin";
setenv("PATH",getenv("PATH") + ":" + ants_bin);


output_folder = '/media/biswal5090pc/Expansion/ABIDE/NYU/whifun_op/ABIDE-1_nyu_whifun_op';%'/media/biswal5090pc/Expansion1/NKI_all_data/whifun_bas1_1400tr_rest_healthy_op';%'/media/biswal5090pc/M/HBN_asd_run-2_whifun_op';%'/media/biswal5090pc/M/HBN_whifun_op_ses-2';
...
for subji=1:n_tot%,parforArg) %par
    disp([num2str(subji) ' out of ' num2str(n_tot)])
    if ~par_on
        tic
    else
        Par.tic;
    ...
    Subj_list(subji)= whifun_preproc_fsl_ants(quality_control_path,Subj_list(subji),...
