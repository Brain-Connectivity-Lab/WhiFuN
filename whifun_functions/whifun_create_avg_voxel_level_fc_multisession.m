...
func_img1_filename = Subj_lists{1}(1).final_func_MNI;%complet
...
if isempty(mat_dir)
    for subji = 1:length(Subj_lists{1})      % For loop on number of participants
        tic
        if d_flag
            disp(['Currently Processing ' Subj_lists{1}(subji).name])
        else
            disp(['Currently Processing ' Subj_lists{1}(subji).name ' ' num2str(subji) ' out of ' num2str(length(Subj_lists{1}))])
        end

        if d_flag
            steps_ = steps_ + 1;
            d.Value = steps_/tot_steps;
            d.Message = ['Creating Voxel-based FC for participant: ' Subj_lists{1}(subji).name];
        ...
        out_voxts_mat_path = fullfile(vox_ts_folder,[Subj_lists{1}(subji).name '_' WM_or_GM '_vox_ts_from_mask_' group_mask_name '.mat']);

        vox_ts_dir = whifun_create_file(over_write,out_voxts_mat_path);
        
        if isempty(vox_ts_dir)
            vox_ts_with_data_idx_all = [];
            vox_ts_sub_sample_with_data_idx_all =[];
            vox_ts_all = [];
            for si = 1:length(Subj_lists)
                now_func_path = Subj_lists{si}(subji).final_func_MNI;%complete_filepath(fullfile(Subj_list(subji).func_folder,[Norm_pre Smooth_pre f_pre Reg_pre Realign_pre Cut_pre Subj_list(subji).func_name]));
                ...
                temp = double(all_timecourses(group_mask_voxels,:));  % getting the signal across time from all white-matter voxels
                vox_ts = temp - mean(temp,2);
                ...
                current_GM_file = dir(Subj_lists{si}(subji).GM_MNI);
                current_WM_file = dir(Subj_lists{si}(subji).WM_MNI);
                current_CSF_file = dir(Subj_lists{si}(subji).CSF_MNI);
                ...
                if si ~= 1
                    vox_ts_with_data_idx_all = intersect(vox_ts_with_data_idx_all,vox_ts_with_data_idx);
                    vox_ts_sub_sample_with_data_idx_all = intersect(vox_ts_sub_sample_with_data_idx_all,vox_ts_sub_sample_with_data_idx);
                else
                    vox_ts_with_data_idx_all = vox_ts_with_data_idx;
                    vox_ts_sub_sample_with_data_idx_all = vox_ts_sub_sample_with_data_idx;
                end
                vox_ts_all = cat(2,vox_ts_all,vox_ts);

            end
            save(out_voxts_mat_path,"vox_ts_all","vox_ts_with_data_idx_all","sub_sample_grid","vox_ts_sub_sample_with_data_idx_all",'group_mask_voxels')
        else
            load(out_voxts_mat_path,"vox_ts_all","vox_ts_with_data_idx_all","sub_sample_grid","vox_ts_sub_sample_with_data_idx_all")
        end
        ...
        switch sub_sample_choose
            case 'Subsample'
                ...
                avg_vox_level_FC(vox_ts_with_data_idx_all, vox_ts_sub_sample_with_data_idx_all) = avg_vox_level_FC(vox_ts_with_data_idx_all, vox_ts_sub_sample_with_data_idx_all) + corr(vox_ts_all(vox_ts_with_data_idx_all,:)', vox_ts_all(sub_sample_grid(vox_ts_sub_sample_with_data_idx_all),:)');
                num_subjs_notnan(vox_ts_with_data_idx_all, vox_ts_sub_sample_with_data_idx_all) = num_subjs_notnan(vox_ts_with_data_idx_all, vox_ts_sub_sample_with_data_idx_all) + 1;
            case 'Use entire FC'
                % current_corr = corr(vox_ts(vox_ts_with_data_idx,:)', vox_ts(vox_ts_with_data_idx,:)');
                % adding the current connectivity matrix to the sum of all participants, for later averaging
                avg_vox_level_FC(vox_ts_with_data_idx_all, vox_ts_with_data_idx_all) = avg_vox_level_FC(vox_ts_with_data_idx_all, vox_ts_with_data_idx_all) + corr(vox_ts_all(vox_ts_with_data_idx_all,:)', vox_ts_all(vox_ts_with_data_idx_all,:)');
                num_subjs_notnan(vox_ts_with_data_idx_all, vox_ts_with_data_idx_all) = num_subjs_notnan(vox_ts_with_data_idx_all, vox_ts_with_data_idx_all) + 1;
            otherwise
                error('Invalid Subsampling option')
        end
        % clear current_corr;
        clear vox_ts_all; clear current_seg_mask; clear vox_ts_with_data_idx_all; clear vox_ts_sub_sample_with_data_idx_all;
        toc
    end
