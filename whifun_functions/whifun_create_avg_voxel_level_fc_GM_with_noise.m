...
if sub_samp == 1
    sub_sample_grid = zeros(size(wm_group_mask));    % defining a grid of 1s and 0s across the image

    sub_sample_grid(2:2:end, 2:2:end, 2:2:end) = 1; sub_sample_grid(1:2:end, 1:2:end, 1:2:end) = 1;
else
    sub_sample_grid = ones(size(wm_group_mask));    % defining a grid of 1s and 0s across the image
end
sub_sample_grid = sub_sample_grid(wm_group_mask_voxels);   % choosing only locations of gray-matter voxels
...
avg_vox_level_FC = zeros(length(gm_group_mask_voxels),length(sub_sample_grid));
num_subjs_notnan = zeros(length(gm_group_mask_voxels),length(sub_sample_grid));
...
out_mat_path = fullfile(cluster_folder,'avg_vox_level_FC_GM_with_noise.mat');
vox_ts_folder = fullfile(cluster_folder,['Vox_ts_from_' gm_group_mask_name '_GM_with_WM_' wm_group_mask_name]);
...
if isempty(mat_dir)
    for subji = 1:length(Subj_list)      % For loop on number of participants
        ...
        if d_flag
            ...
            d.Message = ['Creating Voxel-based GM with noise FC for participant: ' Subj_list(subji).name];

            if d.CancelRequested
                disp('Creation of (GM with noise)-FN terminated by User')
                return
            end
        end
        
        
        out_voxts_mat_path = fullfile(vox_ts_folder,[Subj_list(subji).name '_vox_ts_from_mask_' gm_group_mask_name '_GM_with_noise_' wm_group_mask_name '.mat']);
        ...
        if isempty(vox_ts_dir)
            ...
            wm_vox_ts = double(randn(numel(wm_group_mask_voxels),size(func_data,4)));  % getting the signal across time from all noise voxels
            ...
            gm_vox_ts_with_data_idx = find(var(gm_vox_ts,[],2)~=0 & ~isnan(var(gm_vox_ts,[],2)) & gm_current_seg_mask(gm_group_mask_voxels)==1);
            % if sub_samp
                wm_vox_ts_sub_sample_with_data_idx = find(var(wm_vox_ts(sub_sample_grid,:),[],2)~=0 & ~isnan(var(wm_vox_ts(sub_sample_grid,:),[],2)) & wm_current_seg_mask(wm_group_mask_voxels(sub_sample_grid))==1);
            % else
                wm_vox_ts_sub_with_data_idx = find(var(wm_vox_ts,[],2)~=0 & ~isnan(var(wm_vox_ts,[],2)) & wm_current_seg_mask(wm_group_mask_voxels)==1);
            % end
           
            save(out_voxts_mat_path,"wm_vox_ts","gm_vox_ts","gm_vox_ts_with_data_idx","sub_sample_grid","wm_vox_ts_sub_sample_with_data_idx",'wm_group_mask_voxels','gm_group_mask_voxels')
        else
            load(out_voxts_mat_path,"wm_vox_ts","gm_vox_ts","gm_vox_ts_with_data_idx","sub_sample_grid","wm_vox_ts_sub_sample_with_data_idx")
        end
        ...
        switch sub_sample_choose
            case 'Subsample'
                ...
                avg_vox_level_FC(gm_vox_ts_with_data_idx, wm_vox_ts_sub_sample_with_data_idx) = avg_vox_level_FC(gm_vox_ts_with_data_idx, wm_vox_ts_sub_sample_with_data_idx) + corr(gm_vox_ts(gm_vox_ts_with_data_idx,:)', wm_vox_ts(sub_sample_grid(wm_vox_ts_sub_sample_with_data_idx),:)');
                num_subjs_notnan(gm_vox_ts_with_data_idx, wm_vox_ts_sub_sample_with_data_idx) = num_subjs_notnan(gm_vox_ts_with_data_idx, wm_vox_ts_sub_sample_with_data_idx) + 1;
            case 'Use entire FC'
                % current_corr = corr(vox_ts(vox_ts_with_data_idx,:)', vox_ts(vox_ts_with_data_idx,:)');
                % adding the current connectivity matrix to the sum of all participants, for later averaging
                avg_vox_level_FC(gm_vox_ts_with_data_idx, wm_vox_ts_sub_with_data_idx) = avg_vox_level_FC(gm_vox_ts_with_data_idx, wm_vox_ts_sub_with_data_idx) + corr(gm_vox_ts(gm_vox_ts_with_data_idx,:)', wm_vox_ts(wm_vox_ts_sub_with_data_idx,:)');
                num_subjs_notnan(gm_vox_ts_with_data_idx, wm_vox_ts_sub_with_data_idx) = num_subjs_notnan(gm_vox_ts_with_data_idx, wm_vox_ts_sub_with_data_idx) + 1;
            otherwise
                error('Invalid Subsampling option')
        end
        % clear current_corr;
        clear wm_vox_ts; clear wm_current_seg_mask gm_current_seg_mask; clear gm_vox_ts_with_data_idx; clear wm_vox_ts_sub_sample_with_data_idx;
        toc
    end
...
gm_group_mask_voxels(missing_voxels) = []; avg_vox_level_FC(missing_voxels,:)=[];
