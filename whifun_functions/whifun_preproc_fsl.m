...
%   executes a full preprocessing workflow for a single subject, including 
%   volume discarding, motion correction (MCFLIRT), FD calculation, 
%   anatomical processing (fsl_anat), normalization to MNI space, 
%   nuisance regression, and spatial smoothing.
%
%   INPUTS:
%       quality_control_path - String. Path to the directory where logs and 
%                              QC outputs will be stored.
%       Subj_list_1          - Struct. Contains subject-specific fields:
%                              .name        : Subject ID
%                              .func_folder : Path to functional data
%                              .func_name   : Name of functional NIfTI
%                              .anat_folder : Path to anatomical data
%                              .anat_name   : Name of anatomical NIfTI
%
%   OPTIONAL PARAMETERS (Name-Value Pairs):
%       'over_write'         - Logical. Re-run steps even if files exist (Default: 0).
%       'FSLDIR'             - String. Path to FSL installation.
%       'n_vol_dis'          - Integer. Number of initial volumes to discard (Default: 0).
%       'max_fd'             - Numeric. Framewise Displacement threshold (Default: 0.5).
%       'Reg_CSF'            - Logical. Perform CSF nuisance regression (Default: 0).
%       'motion_reg'         - Logical. Perform motion nuisance regression (Default: 0).
%       'smooth_fwhm'        - Numeric. Smoothing kernel size in mm (Default: 4).
%       'WM_GM_seperate'     - Logical. Use tissue-specific smoothing (Default: 1).
%
%   OUTPUTS:
%       Subj_list_1          - Updated struct containing paths to all 
%                              processed files (e.g., .func_MNI, .GM_MNI).
%
%   DEPENDENCIES:
%       Requires FSL (mcflirt, fsl_anat, epi_reg, applywarp, fslmaths, FAST).
%
%   See also MCFLIRT, FSL_ANAT, WHIFUN_REGRESS_ANY.
%% ---- input parser ----
p = inputParser;
...
if isempty(out_anat_file)
    ...
    cmd = sprintf('fsl_anat -i %s --noseg --nosubcortseg -o %s',in_anat_path, fullfile(fileparts(in_anat_path),anat_preproc_dir));
    fprintf(log_fileID,'#####################################################################################################################\n \n');
    ...
    Subj_list_1.deformation_field = fullfile(fileparts(in_anat_path),[anat_preproc_dir '.anat'],'T1_to_MNI_nonlin_coeff.nii.gz');
else
...
%% Note
% Here i am first going to the MNI space and then doing the other preproc.
% This can be changed later.


%% fsl Func to anat to MNI space
...
mni_full  = fullfile(FSLDIR,'data','standard','MNI152_T1_2mm.nii.gz');
Subj_list_1.MNI_template = mni_full;
out_func_path = fullfile(now_func_path.folder,['anat_space_mean_' now_func_path.name]);
...
if isempty(out_func_file)
    ...
    % func reg
    cmd = sprintf('epi_reg --epi=%s --t1=%s --t1brain=%s --out=%s',meanfunc,fullfile(fileparts(in_anat_path),[anat_preproc_dir '.anat'],'T1_biascorr.nii.gz'),fullfile(fileparts(in_anat_path),[anat_preproc_dir '.anat'],'T1_biascorr_brain.nii.gz'),fullfile(now_func_path.folder,['anat_space_mean_' now_func_path.name]));
    fprintf(log_fileID,'#####################################################################################################################\n \n');
    fprintf(log_fileID, 'function 2 Anat epi_reg\n');
    fprintf(log_fileID,'%s',  sprintf('[CMD] %s\n', cmd));
    [s,w] = system(cmd);
    if s~=0
        error('Command failed (exit %d): %s\nOutput:\n%s', s, cmd, w);
...
% func to mni
[~,func_name,~] = fileparts(out_func_path);
...
if isempty(out_func_file)
    cmd = sprintf('applywarp --ref=%s --in=%s --out=%s --warp=%s --premat=%s', mni_full, in_func_path,out_func_path,fullfile(fileparts(in_anat_path),[anat_preproc_dir '.anat'],'T1_to_MNI_nonlin_coeff.nii.gz'),fullfile(now_func_path.folder,[func_name '.mat']) );
    fprintf(log_fileID,'#####################################################################################################################\n \n');
...
out_anat_path = fullfile(fileparts(in_anat_path),'T1_to_MNI_nonlin_pve_1.nii.gz');

out_anat_file = whifun_create_file(over_write,out_anat_path);
if isempty(out_anat_file)
    [gm_prob_path, wm_prob_path, csf_prob_path,t1_brain_mask] = whifun_fsl_fast_seg(in_anat_path,fileparts(in_anat_path),true);
    fprintf(log_fileID,'#####################################################################################################################\n \n');
    ...
    gm_prob_path = fullfile(fileparts(in_anat_path),'T1_to_MNI_nonlin_pve_1.nii.gz');
    wm_prob_path = fullfile(fileparts(in_anat_path),'T1_to_MNI_nonlin_pve_2.nii.gz');
    csf_prob_path = fullfile(fileparts(in_anat_path),'T1_to_MNI_nonlin_pve_0.nii.gz');
    t1_brain_mask = fullfile(fileparts(in_anat_path),'T1_to_MNI_nonlin_brain_mask.nii.gz');
    Subj_list_1.GM_MNI = gm_prob_path;
    Subj_list_1.WM_MNI = wm_prob_path;
    Subj_list_1.CSF_MNI = csf_prob_path;
