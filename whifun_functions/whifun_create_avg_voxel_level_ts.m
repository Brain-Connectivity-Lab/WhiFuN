function whifun_create_avg_voxel_level_ts(cluster_folder,Subj_list,group_mask_path,sub_sample_choose,WM_or_GM,field,over_write,d_flag,d,steps_,tot_steps)
...
func_img1_filename = Subj_list(1).(field);%complet
...
if isempty(mat_dir)
    for subji = 1:length(Subj_list)      % For loop on number of participants
        ...
        if isempty(vox_ts_dir)
            now_func_path = Subj_list(subji).(field);%complete_filepath(fullfile(Subj_list(subji).func_folder,[Norm_pre Smooth_pre f_pre Reg_pre Realign_pre Cut_pre Subj_list(subji).func_name]));
