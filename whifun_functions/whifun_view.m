...
%   tiled layout showing Coronal (XZ), Sagittal (YZ), and Axial (XY) planes.
%   If the input is a 3D volume, the 4th panel displays the current voxel value.
%   If the input is a 4D volume (e.g., fMRI), the 4th panel plots the entire 
%   BOLD time series for the selected voxel with a real-time frame indicator.
%
%   WHIFUN_VIEW(vol, M) attaches a 4x4 affine transformation matrix (M). This 
%   activates additional interactive UI edit boxes displaying real-time MNI 
%   coordinates. Users can input target MNI coordinates directly to jump the 
%   crosshairs to that anatomical location.
%
%   Input Flexibility:
%       If 'vol' is passed as a string or char file path to a NIfTI image, the 
%       function automatically reads the volume and extracts its spatial 
%       transformation matrix (M) from the header information.
%
...
%
%   Dependencies:
%       Requires 'whifun_convert_coords' if an affine matrix is supplied or 
%       automatically loaded from a NIfTI path.
%
%   Inputs:
%       vol - A 3D matrix (X x Y x Z), 4D matrix (X x Y x Z x T), or a string/char 
%             specifying the file path to a valid NIfTI file.
%       M   - (Optional) 4x4 affine transformation matrix mapping voxels to MNI 
%             space. Automatically extracted if 'vol' is a file path.
%
%   Examples:
%       % Example 1: View a NIfTI file directly (auto-loads MNI space)
%       whifun_view('C:\Data\wfunc.nii');
%
%       % Example 2: View a raw 3D matrix without MNI space mapping
%       imgData = rand(91, 109, 91); 
%       whifun_view(imgData);
%
%   Author: Pratik Jain
