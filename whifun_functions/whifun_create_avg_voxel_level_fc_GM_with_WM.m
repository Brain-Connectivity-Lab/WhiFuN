...
out_mat_path = fullfile(cluster_folder,'avg_vox_level_FC_GM_with_WM.mat');
...
if isempty(mat_dir)
    for subji = 1:length(Subj_list)      % For loop on number of participants
        ...
        if d_flag
            ...
            d.Message = ['Creating Voxel-based GM with WM FC for participant: ' Subj_list(subji).name];
        ...
        out_voxts_mat_path = fullfile(vox_ts_folder,[Subj_list(subji).name '_vox_ts_from_mask_' gm_group_mask_name '_GM_with_WM_' wm_group_mask_name '.mat']);
        ...
        if isempty(vox_ts_dir)
            ...
            wm_vox_ts = double(all_timecourses(wm_group_mask_voxels,:));  % getting the signal across time from all white-matter voxels
