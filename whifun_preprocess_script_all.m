...
for subji=1:length(Subj_list_all)
    ...
    if ~isfield(Subj_list_all,'manual_ex')
        Subj_list_all(subji).manual_ex = 0;
    elseif isempty(Subj_list_all(subji).manual_ex)
        Subj_list_all(subji).manual_ex = 0;
    end
    if ~Subj_list_all(subji).manual_ex
        try
            now_func_path = dir(fullfile(Subj_list_all(subji).func_folder,Subj_list_all(subji).func_name)) ;
            if isempty(now_func_path)
                now_func_path = dir(fullfile(Subj_list_all(subji).func_folder,[Subj_list_all(subji).func_name '.gz'])) ;

                if isempty(now_func_path)
                    msg = ['No func data found for participant: ' Subj_list_all(subji).name];
                    report(p) = msg;
                    p = p+1;
                    disp([msg 'trying to open: ' fullfile(Subj_list_all(subji).func_folder,Subj_list_all(subji).func_name) ' or ' fullfile(Subj_list_all(subji).func_folder,[Subj_list_all(subji).func_name '.nii*'])])
                    %                                 Subj_list_all(subji). = 0;
                    if strcmp(Subj_list_all(subji).func_folder,fullfile(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name, app.func_folder_name))
                        if ~isfolder(complete_filepath(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name))
                            msg = ['The intermediate folder name "' char(app.comm_sess_name) '" not found'];
                            report(p) = msg;
                            p = p+1;
                        elseif ~isfolder(complete_filepath(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name,app.func_folder_name))
                            msg = ['The func folder name "' char(app.func_folder_name) '" not found.'];
                            report(p) = msg;
                            p = p+1;
                        else
                            msg = ['The func image name "' char(app.func_data_name) '.gz' '" not found'];
                            report(p) = msg;
                            p = p+1;
                        end
                    end
            ...
            if ~isempty(now_func_path)
                ...
                Subj_list_all(subji).func_folder = now_func_path(1).folder;
                Subj_list_all(subji).func_name = now_func_path(1).name;
                ...
                if ~sum(isinf(v_func.PixelDimensions))
                    voxel_func(subji,:) = v_func.PixelDimensions(1:3);                                 % voxel size
                else
                    voxel_func(subji,:) = nan;
                end

                if ~sum(isinf(v_func.ImageSize(4)))
                    n_image(subji,1) = v_func.ImageSize(4);                                  % number of images (time points)
                    Subj_list_all(subji).nt = v_func.ImageSize(4);
                else
                    n_image(subji,1) = nan;
                    Subj_list_all(subji).nt = nan;
                end

                if ~sum(isinf(v_func.PixelDimensions(1)))
                    Subj_list_all(subji).x_func = v_func.PixelDimensions(1);
                else
                    Subj_list_all(subji).x_func = nan;
                end

                if ~sum(isinf(v_func.PixelDimensions(2)))
                    Subj_list_all(subji).y_func = v_func.PixelDimensions(2);
                else
                    Subj_list_all(subji).y_func = nan;
                end

                if ~sum(isinf(v_func.PixelDimensions(3)))
                    Subj_list_all(subji).z_func = v_func.PixelDimensions(3);
                else
                    Subj_list_all(subji).z_func = nan;
                end

            else
                no_func(subji) = 1;
                Subj_list_all(subji).error = 1;
            ...
            disp(['Preprocessing has encountered errors in Data Initial check (functional) for ' Subj_list_all(subji).name ', I have saved the variables in the participant folder :-) '])
            disp('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')

            write_error(exception,quality_control_path, Subj_list_all(subji).name)                % write error to text file and display
            Subj_list_all(subji).error = 1;
            continue
        end
        ...
        try
            now_anat_path = dir(fullfile(Subj_list_all(subji).anat_folder,Subj_list_all(subji).anat_name)) ;
            if isempty(now_anat_path)
                now_anat_path = dir(fullfile(Subj_list_all(subji).anat_folder,[Subj_list_all(subji).anat_name '.gz'])) ;

                if isempty(now_anat_path)
                    msg = ['No anat data found for participant: ' Subj_list_all(subji).name];
                    report(p) = msg;
                    p = p+1;
                    disp([msg 'trying to open: ' fullfile(Subj_list_all(subji).anat_folder,Subj_list_all(subji).anat_name) ' or ' fullfile(Subj_list_all(subji).anat_folder,[Subj_list_all(subji).anat_name '.gz'])])
                    if strcmp(Subj_list_all(subji).anat_folder,fullfile(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name, app.anat_folder_name))
                        if ~isfolder(complete_filepath(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name))
                            msg = ['The intermediate folder name "' char(app.comm_sess_name) '" not found'];
                            report(p) = msg;
                            p = p+1;
                        elseif ~isfolder(complete_filepath(Subj_list_all(subji).folder, Subj_list_all(subji).name,app.comm_sess_name,app.anat_folder_name))
                            msg = ['The anat folder name "' char(app.anat_folder_name) '" not found.'];
                            report(p) = msg;
                            p = p+1;
                        else
                            msg = ['The anat image name "' char(app.anat_data_name) '.gz' '" not found'];
                            report(p) = msg;
                            p = p+1;
                        end
                    end
            ...
            if ~isempty(now_anat_path)
                ...
                Subj_list_all(subji).anat_folder = now_anat_path(1).folder;
                Subj_list_all(subji).anat_name = now_anat_path(1).name;
                v_anat = niftiinfo(fullfile(now_anat_path(1).folder,now_anat_path(1).name));
                voxel_anat(subji,:) = v_anat.PixelDimensions(1:3);                                % voxel size

                if ~sum(isinf(v_anat.PixelDimensions))
                    voxel_anat(subji,:) = v_anat.PixelDimensions(1:3);                                 % voxel size
                else
                    voxel_anat(subji,:) = nan;
                end

                if ~sum(isinf(v_anat.PixelDimensions(1)))
                    Subj_list_all(subji).x_Anat = v_anat.PixelDimensions(1);
                else
                    Subj_list_all(subji).x_Anat = nan;
                end

                if ~sum(isinf(v_anat.PixelDimensions(2)))
                    Subj_list_all(subji).y_Anat = v_anat.PixelDimensions(2);
                else
                    Subj_list_all(subji).y_Anat = nan;
                end

                if ~sum(isinf(v_anat.PixelDimensions(3)))
                    Subj_list_all(subji).z_Anat = v_anat.PixelDimensions(3);
                else
                    Subj_list_all(subji).z_Anat = nan;
                end
            ...
            disp('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
            disp(['Preprocessing has encountered errors in Data Initial check (anatomical) for ' Subj_list_all(subji).name ', I have saved the variables in the participant folder :-) '])
            disp('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')

            write_error(exception,quality_control_path, Subj_list_all(subji).name)            % write error to text file and display
            Subj_list_all(subji).error = 1;                                                        % % Remove participant from further preprocessing
        end
        ...
        try
            ...
            if ~sum(isinf(v_func.PixelDimensions(4)))
                tr(subji,1) = v_func.PixelDimensions(4);                                 % voxel size
                Subj_list_all(subji).TR = v_func.PixelDimensions(4);
            else
                tr(subji,1) = nan;
                Subj_list_all(subji).TR = nan;
            end
    ...
    if ~isfield(Subj_list_all,'time_preprocess_min')
        Subj_list_all(subji).time_preprocess_min = 0;
    elseif isempty(Subj_list_all(subji).time_preprocess_min)
        Subj_list_all(subji).time_preprocess_min = 0;
    end
end
...
if nnz(no_anat) || nnz(no_func)
    msg_test = ['Total Number of participants found in the participant folder was ' num2str(length(Subj_list_all)) ' but ' num2str(nnz(no_func)+nnz(no_anat)) ' were removed as they did not have either the func or anat files'];
    msgbox(msg_test,'Missing anat or func images')
    mis_data = 1;
end
...
saveas(fh,fullfile(quality_control_path,'Q1a_scanning_parameters.png'));
%             close gcf

fh1 = figure('DefaultAxesFontSize',14,'Visible','off','Position',[0,0,2000,1000]);
...
if mis_data == 1
    annstr = "Because there were missing files, the parameters corresponding to those are 0";
    annpos = [0.02 0.9 0.1 0.1]; % annotation position in figure coordinates
    annotation('textbox',annpos,'string',annstr);
end
saveas(fh1,fullfile(quality_control_path,'Q1a_scanning_parameters_histogram.png'));
...
% reference
comm_subj_name = app.comm_subj_name;
comm_sess_name= app.comm_sess_name;
func_folder_name = app.func_folder_name;
anat_folder_name = app.anat_folder_name;
func_data_name = app.func_data_name;       % %   Functional data name
anat_data_name = app.anat_data_name;    % %   Anatomical data name
bids_check = 0;
